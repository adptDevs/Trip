...
function add_row($cn){
	...
	$sql = 	"INSERT INTO brochures(idpos,kit,abbr,name,activeflag)
			VALUES ('".$_POST["c0"]."',
					'".$_POST["c1"]."',
					'".$_POST["c2"]."',
					'".$_POST["c3"]."',
...
function update_row($cn){
	$sql = 	"UPDATE brochures SET  idpos='".$_POST["c0"]."',
					kit='".$_POST["c1"]."',
					abbr='".$_POST["c2"]."',
					name='".$_POST["c3"]."',
					activeflag= '".$_POST["c4"]."'

			WHERE idpos=".$_POST["gr_id"];
	$res = sqlsrv_query($cn, $sql);
...
function delete_row($cn){
	...
	$d_sql = "DELETE FROM brochures WHERE idpos=".$_POST["gr_id"];
